...
export class EditTechnicalRequestHttpService {
  constructor(private http: HttpClient) {}

  execute(id: string, data: TechnicalRequestModel): Observable<string> {
    const headers = new HttpHeaders({ 'Content-Type': 'application/json' });
    return this.http.put<string>(
      `https://TechnicalRequestNodeAPI.darrenatienza.repl.co/${id}`,
      data,
      { headers: headers }
    );
  }
